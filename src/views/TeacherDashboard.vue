  ...
  <v-app>
    ...
    <v-main>
      <v-container>
        ...
        <v-row v-if="loadingUser" class="text-center">
          <v-col cols="12">
        ...
        <!-- Le tableau de bord n’apparaît qu’une fois l’utilisateur chargé -->
        <template v-else>
          <v-row>
            <v-col cols="12">
              <v-card class="mb-6">
...
const loadMyCourses = async () => {
  ...
  try {
    myCourses.value = await AttendanceService.getTeacherCourses(user.value.id)
  } catch (error) {
...
const generateQR = async () => {
  ...
  try {
    ...
    qrCodeSVG.value = await QRCode.toString(qr_url, { type: 'svg', width: 200, margin: 2, color: { dark: '#003366', light: '#FFFFFF' } })
    timeLeft.value = 5 * 60
    startTimer()
    presences.value = []
...
const refreshPresences = async () => {
  ...
  try {
    presences.value = await AttendanceService.getSessionAttendance(currentSession.value.id)
  } catch (error) {
...
const startTimer = () => {
  ...
  timer.value = setInterval(() => {
    if (timeLeft.value > 0) {
      timeLeft.value--
      if (timeLeft.value % 10 === 0) refreshPresences()
    } else {
